---- Analyse Profil Client Famille de Produits 

-- Parametre des dates 
SET dtdeb = Date('2024-01-01');
SET dtfin = DAte('2024-01-30');
SET tag_etud_F1='Bermuda'; -- EXEMPLE DE FAMILLE BERMUDA
SET tag_etud_F2='ZZZNULL';
SET tag_etud_F3='ZZZNULL';
...
WITH tab0 AS (SELECT DISTINCT CODE_CLIENT AS Id_Client, LIB_FAMILLE_ACHAT AS ref_achat,
...
AND LIB_FAMILLE_ACHAT IN ($tag_etud_F1, $tag_etud_F2 , $tag_etud_F3) -- listes des familles a analyser on peut analyse un ou plusieurs familles  
...
tab1 AS (SELECT Id_Client, ref_achat,
...
SELECT * FROM DATA_MESH_PROD_CLIENT.WORK.IDCLT_ETUDE ;
  ...
  *  
  -- il faut avoir une table REFO avec l'ensemble des references à prendre en compte 
...
FROM DHB_PROD.DNR.DN_VENTE vd
where vd.date_ticket BETWEEN DATE($dtdeb) AND DATE($dtfin) 
AND vd.CODE_REFERENCE IN (SELECT DISTINCT CODE_REFERENCE FROM TAB_REFCO)  -- listes des familles a analyser on peut analyse un ou plusieurs familles  
  and vd.ID_ORG_ENSEIGNE IN ($ENSEIGNE1 , $ENSEIGNE2)
  and vd.code_pays IN  ($PAYS1 ,$PAYS2) AND vd.code_client IS NOT NULL AND vd.code_client !='0' ),
tab1 AS (SELECT Id_Client, ref_achat,
Count(DISTINCT CASE WHEN Qte_pos>0 THEN id_ticket end ) AS nb_ticket,
