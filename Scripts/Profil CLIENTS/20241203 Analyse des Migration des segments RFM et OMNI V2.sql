--- Analyse Migration des segments entre le 01 janvier 2024 et le 31 Décembre 2024 
...
-- Analyse Vision Segment RFM 

CREATE OR REPLACE TEMPORARY TABLE DATA_MESH_PROD_CLIENT.WORK.t_segment_RFM AS
...
SELECT a.CODE_CLIENT, a.ID_MACRO_SEGMENT_DEB, a.LIB_MACRO_SEGMENT_DEB, 
CASE WHEN a.ID_MACRO_SEGMENT_DEB = '01' THEN '01_VIP' 
     WHEN a.ID_MACRO_SEGMENT_DEB = '02' THEN '02_TBC'
     WHEN a.ID_MACRO_SEGMENT_DEB = '03' THEN '03_BC'
     WHEN a.ID_MACRO_SEGMENT_DEB = '04' THEN '04_MOY'
     WHEN a.ID_MACRO_SEGMENT_DEB = '05' THEN '05_TAP'
     WHEN a.ID_MACRO_SEGMENT_DEB = '06' THEN '06_TIEDE'
     WHEN a.ID_MACRO_SEGMENT_DEB = '07' THEN '07_TPURG'
     WHEN a.ID_MACRO_SEGMENT_DEB = '09' THEN '08_NCV'
     WHEN a.ID_MACRO_SEGMENT_DEB = '08' THEN '09_NAC'
     WHEN a.ID_MACRO_SEGMENT_DEB = '10' THEN '10_INA12'
     WHEN a.ID_MACRO_SEGMENT_DEB = '11' THEN '11_INA24'
...
FROM DATA_MESH_PROD_CLIENT.WORK.t_segment_RFM a
...
CREATE OR REPLACE TEMPORARY TABLE DATA_MESH_PROD_CLIENT.WORK.Vision_glb_RFM_2023  AS
SELECT * FROM 
...
Select * FROM DATA_MESH_PROD_CLIENT.WORK.Vision_glb_RFM_2023 ORDER BY 1,2 ; 


-- Analyse Vision Segment OMNI

CREATE OR REPLACE TEMPORARY TABLE DATA_MESH_PROD_CLIENT.WORK.t_segment_OMNI AS
...
WHERE DATE_DEBUT <= DATE($dtfin)
AND (DATE_FIN > DATE($dtfin) OR DATE_FIN IS NULL) AND a.code_client IS NOT NULL AND a.code_client !='0')
SELECT a.CODE_CLIENT, a.LIB_SEGMENT_OMNI_DEB,
CASE WHEN a.LIB_SEGMENT_OMNI_DEB='OMNI' THEN '03_OMNI'
       WHEN a.LIB_SEGMENT_OMNI_DEB='MAG' THEN '01_MAG'
       WHEN a.LIB_SEGMENT_OMNI_DEB='WEB' THEN '02_WEB'
       ELSE '99_NOSEG' END AS SEGMENT_OMNI_DEB,
 b.LIB_SEGMENT_OMNI_FIN,
  CASE WHEN b.LIB_SEGMENT_OMNI_FIN='OMNI' THEN '03_OMNI'
       WHEN b.LIB_SEGMENT_OMNI_FIN='MAG' THEN '01_MAG'
       WHEN b.LIB_SEGMENT_OMNI_FIN='WEB' THEN '02_WEB'
       ELSE '99_NOSEG' END AS SEGMENT_OMNI_FIN
FROM segomni_deb a
...
SELECT a.* ,b.nb_ticket ,b.CA ,b.Qte ,b.Marge ,b.Mnt_remise
FROM DATA_MESH_PROD_CLIENT.WORK.t_segment_OMNI a
LEFT JOIN DATA_MESH_PROD_CLIENT.WORK.tab_tickets b ON a.CODE_CLIENT=b.CODE_CLIENT ; 
...
SELECT '01-Client Global' AS typo, *  FROM tagbd
pivot (SUM(nb_clt_glb) for SEGMENT_OMNI_FIN in ('01_MAG','02_WEB','03_OMNI','99_NOSEG'));
...
SELECT '02-Client Actif' AS typo, *  FROM tagbd
pivot (SUM(nb_clt_actif) for SEGMENT_OMNI_FIN in ('01_MAG','02_WEB','03_OMNI','99_NOSEG'));
...
SELECT '03-CA/Clt' AS typo, *  FROM tagbd
pivot (SUM(CA_par_clt_glb) for SEGMENT_OMNI_FIN in ('01_MAG','02_WEB','03_OMNI','99_NOSEG'));
...
SELECT '04-Fréquence' AS typo, *  FROM tagbd
pivot (SUM(freq_clt_glb) for SEGMENT_OMNI_FIN in ('01_MAG','02_WEB','03_OMNI','99_NOSEG'));
...
SELECT '05-Panier Moy' AS typo, *  FROM tagbd
pivot (SUM(panier_clt_glb) for SEGMENT_OMNI_FIN in ('01_MAG','02_WEB','03_OMNI','99_NOSEG'));


CREATE OR REPLACE TEMPORARY TABLE DATA_MESH_PROD_CLIENT.WORK.Vision_glb_OMNI_2023  AS
SELECT * FROM 
...
SELECT * FROM DATA_MESH_PROD_CLIENT.WORK.Vision_glb_OMNI_2023 ;
