...
,SUM(MONTANT_TTC ) AS CA_glb_hist
,SUM(QUANTITE_LIGNE ) AS qte_glb_hist
,SUM(MONTANT_MARGE_SORTIE ) AS Marge_glb_hist	
,Count(DISTINCT CASE WHEN PERIMETRE_achat='WEB' THEN id_ticket end) AS nb_tick_web_hist
,SUM(CASE WHEN PERIMETRE_achat='WEB' THEN MONTANT_TTC END ) AS CA_web_hist
,SUM(CASE WHEN PERIMETRE_achat='WEB' THEN QUANTITE_LIGNE END ) AS qte_web_hist
,SUM(CASE WHEN PERIMETRE_achat='WEB' THEN MONTANT_MARGE_SORTIE END ) AS Marge_web_hist
FROM DATA_MESH_PROD_CLIENT.WORK.tab_act_histo
GROUP BY 1,2; 

NB_centre, NB_centre_mag, NB_centre_web, nb_tick_glb_hist, CA_glb_hist, qte_glb_hist, Marge_glb_hist,
nb_tick_web_hist, CA_web_hist, qte_web_hist, Marge_web_hist
...
WITH tab1 AS (
SELECT ens_ferm, idmag_ferm, type_emplacement, lib_magasin, type_ferm, date_fermeture_public
  ...
  ,SUM(CASE WHEN code_client IS NOT NULL AND code_client !='0' AND PERIMETRE_achat='WEB' THEN MONTANT_MARGE_SORTIE END) AS Marge_clt_web
  FROM tab_ticket_18mthnext
GROUP BY ens_ferm, idmag_ferm, type_emplacement, lib_magasin, type_ferm, date_fermeture_public),
...
WITH tab1 AS (
SELECT ens_ferm, idmag_ferm, type_emplacement, lib_magasin, type_ferm, date_fermeture_public, MAG_CLIENT
  ,MAX(CASE WHEN code_client IS NOT NULL AND code_client !='0' AND delai_reachat IS NOT NULL THEN periode_etud END) AS periode_etud_max
  ,MAX(CASE WHEN code_client IS NOT NULL AND code_client !='0' AND delai_reachat IS NOT NULL THEN delai_reachat END) AS delai_reachat_max
  ,Count(DISTINCT id_client ) AS nb_client_potentiel
  ,Count(DISTINCT CASE WHEN code_client IS NOT NULL AND code_client !='0' THEN code_client END ) AS nbclt_actif
  ...
  ,SUM(CASE WHEN code_client IS NOT NULL AND code_client !='0' AND PERIMETRE_achat='WEB' THEN MONTANT_MARGE_SORTIE END) AS Marge_clt_web
  FROM tab_ticket_18mthnext
GROUP BY ens_ferm, idmag_ferm, type_emplacement, lib_magasin, type_ferm, date_fermeture_public, MAG_CLIENT),
tclt AS (SELECT DISTINCT ens_ferm, idmag_ferm, code_client FROM tab_ticket_18mthnext WHERE code_client IS NOT NULL AND code_client !='0'), 
tab_histo AS (
...
WITH react AS (SELECT DISTINCT ens_ferm, idmag_ferm,type_emplacement, lib_magasin AS lib_magasin_ferm, type_ferm, date_fermeture_public, nb_client_potentiel, nbclt_actif FROM DATA_MESH_PROD_CLIENT.WORK.Stat_react_client_mag ORDER BY 1,2 )
